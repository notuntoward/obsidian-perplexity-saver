...
This plugin solves one narrow, specific problem: the user (Scott) wants to
capture Perplexity.ai research dialogs into Obsidian in a way that is clearly
segregated from his own hand-written notes, while still being conveniently
linked from the note he is actively writing.

If you are an AI assistant asked to modify, debug, or extend this plugin,
understand the following constraints before making changes:

1. **Do not reintroduce Templater as a dependency.** An earlier version of this
   workflow used the QuickAdd + Templater plugin combination
   (`tp.file.folder(true)`), which caused Obsidian to freeze/hang unpredictably
   due to Templater's `tp` object not always being initialized when called from
   QuickAdd. This plugin was built specifically to remove that fragile
   dependency by using Obsidian's own `TFile.parent.path` instead. Do not
   suggest re-adding Templater unless explicitly asked.

2. **Frontmatter must be merged, not overwritten.** The user also runs the
   community plugin "Front Matter Timestamps," which automatically inserts
   `created`/`modified` fields into new notes. This plugin's tagging step uses
   `app.fileManager.processFrontMatter()` specifically because it reads-modifies-
   writes the frontmatter object atomically, merging into whatever the
   timestamps plugin already wrote. Never replace this with raw string
   concatenation of a `---\ntags:...\n---` block, since that caused
   duplicate/broken frontmatter in an earlier iteration of this workflow.

3. **The Perplexity-side capture is out of scope for this plugin.** A separate
   Tampermonkey userscript (not part of this repo) runs in the browser, triggers
   the "Complexity" browser extension's native export feature on perplexity.ai,
   and copies clean Markdown to the system clipboard, prefixed with a visible
   `[Perplexity](url)` link. This plugin's only job is to take whatever is
   already on the clipboard and file it correctly inside Obsidian — it should
   never attempt to fetch or scrape Perplexity content directly.

4. **Minimal manual input is a hard requirement.** The user explicitly wants as
   few manual steps as possible. The only acceptable manual step in this plugin
   is typing a filename when prompted. Do not add additional prompts,
   confirmation dialogs, or settings screens unless the user asks for them.

5. **Folder creation must be idempotent per note-folder, not global.** The
   `ai-searches` subfolder is created relative to whatever folder the currently
   active note lives in — each topic folder gets its own independent
   `ai-searches` subfolder, created on first use and reused afterward. This is
   intentional; do not consolidate all captures into one global folder.

### Code-style / compliance notes

This repo is scaffolded from the `obsidian-plugin-template`, which enforces a
strict ESLint config (`eslint-plugin-obsidianmd`, zero warnings) and a
`tsc --noEmit` type-check as part of `npm run build`. When editing `src/main.ts`,
keep the code lint-clean. In particular:

- `app.fileManager.processFrontMatter()` was introduced in Obsidian **1.4.4**, so
  `manifest.json` `minAppVersion` must stay at `1.4.4` or higher.
- Prefer native Obsidian APIs (e.g. `processFrontMatter`, `generateMarkdownLink`,
  `normalizePath`) over raw string/YAML manipulation.
