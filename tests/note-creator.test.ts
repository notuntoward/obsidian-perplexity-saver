...
describe("createPerplexityNote", () => {
	...
	it("relinks sources when autoRelinkSources is true in createPerplexityNote", async () => {
	...
	it("does not relink sources when autoRelinkSources is false in createPerplexityNote", async () => {
		const result = await createPerplexityNote({
			app: mockApp,
			activeFile: mockActiveFile,
			clipboardContent: "**You**\n\nTell me about climate.\n\n**AI answer**\n\nClimate change is real[^1_1].\n\n# Sources\n\n[^1_1]: [Climate Study Title](https://example.com/climate)",
